...
public class Board
{
    ...
    internal void FillDivisibleByThree()
    {
        int total = boardSizeX * boardSizeY;
        Array allTypes = Enum.GetValues(typeof(NormalItem.eNormalType));

        int groupsTotal = total - (total % 3);
        int groupsCount = groupsTotal / 3;

        if (total % 3 != 0)
        {
            Debug.LogWarning(string.Format(
                "Board has {0} cells which is not divisible by 3. {1} extra cell(s) " +
                "will duplicate an existing group so the board can still be filled.",
                total, total % 3));
        }

        List<NormalItem.eNormalType> bag = new List<NormalItem.eNormalType>();

        // make sure every fish type is placed at least once.
        List<NormalItem.eNormalType> shuffledTypes = allTypes.Cast<NormalItem.eNormalType>()
            .OrderBy(t => UnityEngine.Random.value)
            .ToList();

        int groupsAdded = 0;
        foreach (NormalItem.eNormalType type in shuffledTypes)
        {
            if (groupsAdded >= groupsCount) break;
        ...
        if (groupsAdded < shuffledTypes.Count)
        {
            Debug.LogWarning(string.Format(
                "Board is too small to fit all {0} fish types with groups of 3. Only {1} type(s) were placed.",
                shuffledTypes.Count, groupsAdded));
        }

        while (groupsAdded < groupsCount)
        {
            NormalItem.eNormalType type = (NormalItem.eNormalType)allTypes.GetValue(UnityEngine.Random.Range(0, allTypes.Length));
            bag.Add(type);
            bag.Add(type);
            bag.Add(type);
            groupsAdded++;
        ...
        while (bag.Count < total)
        {
            bag.Add(bag[UnityEngine.Random.Range(0, bag.Count)]);
        }
        ...
        for (int i = bag.Count - 1; i > 0; i--)
        {
            ...
            NormalItem.eNormalType tmp = bag[i];
            bag[i] = bag[j];
            bag[j] = tmp;
        }
    ...
    internal Cell FindCellOfType(NormalItem.eNormalType type)
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (item != null && item.ItemType == type) return cell;
            }
        }
    ...
    internal Cell FindCellOfTypeNotIn(HashSet<NormalItem.eNormalType> excludedTypes)
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                NormalItem item = cell.Item as NormalItem;
                if (item != null && !excludedTypes.Contains(item.ItemType)) return cell;
            }
        }
    ...
    internal Cell FindAnyCellWithItem()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
